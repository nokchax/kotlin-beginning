...
## 문자열
문자열은 `String` 타입으로 표현한다. 인반적으로 문자열 값은 큰따옴표 `"`로 묶인 문자의 연속이다.
```kotlin
val str = "abcd 123"
```

문자열의 요소는 문자이며, `s[i]` 인덱싱 작업을 통해 접근할 수 있다. `for` 루프문으로 통해 이 문자들을 반복할수 있다.
```kotlin
for (c in str) {
  println(c)
}
```
문자열은 불변이다. 한번 초기화 하면 값을 변경할수도 새 값을 할당할 수도 없다. 문자열을 변환하는 모든 연산은 기존의 문자열은 그대로 두고 연산의 결과로 새로운 `String` 객체를 반환한다.
...
println(str)
```
문자열을 연결하기 위해 `+` 연산자를 사용한다. 이 연산자는 표현식의 첫번째 요소가 문자열인 경우, 문자열과 다른 타입의 값을 연결하도록 동작하기도 한다.
```kotlin
val s = "abc" + 1
println(s + "def")
```
대부분의 경우 문자열 연결보다는 문자열 템플릿, raw 문자열을 사용하는 것이 선호된다.

### 문자열 리터럴
코틀린은 두가지 문자열 리터럴이 있다.
- escaped 문자를 포함하고 있는 escaped 문자열
- newline 과 자유로운 텍스트를 포함하는 raw 문자열
